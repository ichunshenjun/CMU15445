...
void TableLockTest1() {
  ...
  for (int i = 0; i < num_oids; i++) {
    threads.emplace_back(std::thread{task, i}); // NOLINT
  }
...
TEST(LockManagerTest, CompatibilityTest) {
  ...
  for (int i = 0; i < num_oids; i++) {
    if (i % 3 == 0) {
      threads.emplace_back(std::thread{task_exclusive, i}); // NOLINT
    } else {
      threads.emplace_back(std::thread{task_shared, i}); // NOLINT
    }
  }
