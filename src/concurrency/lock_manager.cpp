...
auto LockManager::LockTable(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool { 
  ...
  for(auto request:lock_request_queue->request_queue_){
    if(request->txn_id_==txn->GetTransactionId()){
      ...
      if((request->lock_mode_!=LockMode::INTENTION_SHARED||(lock_mode!=LockMode::SHARED&&lock_mode!=LockMode::EXCLUSIVE&&lock_mode!=LockMode::INTENTION_EXCLUSIVE&&lock_mode!=LockMode::SHARED_INTENTION_EXCLUSIVE))&&
      (request->lock_mode_!=LockMode::SHARED||(lock_mode!=LockMode::EXCLUSIVE&&lock_mode!=LockMode::SHARED_INTENTION_EXCLUSIVE))&&(request->lock_mode_!=LockMode::INTENTION_EXCLUSIVE||(lock_mode!=LockMode::EXCLUSIVE&&lock_mode!=LockMode::SHARED_INTENTION_EXCLUSIVE))&&(request->lock_mode_!=LockMode::SHARED_INTENTION_EXCLUSIVE||lock_mode!=LockMode::EXCLUSIVE)){
        lock_request_queue->latch_.unlock();
        txn->SetState(TransactionState::ABORTED);
...
auto LockManager::UnlockTable(Transaction *txn, const table_oid_t &oid) -> bool { 
  ...
  if(!(s_row_lock_set->find(oid)==s_row_lock_set->end()||s_row_lock_set->at(oid).empty())||!(x_row_lock_set->find(oid)==x_row_lock_set->end()||x_row_lock_set->at(oid).empty())){
    table_lock_map_latch_.unlock();
    txn->SetState(TransactionState::ABORTED);
  ...
  for(auto lock_request:lock_request_queue->request_queue_){ // NOLINT
    if(lock_request->txn_id_==txn->GetTransactionId()&&lock_request->granted_){
      ...
      lock_request_queue->cv_.notify_all();
      lock_request_queue->latch_.unlock();
      if(txn->GetIsolationLevel()==IsolationLevel::REPEATABLE_READ&&(lock_request->lock_mode_==LockMode::SHARED||lock_request->lock_mode_==LockMode::EXCLUSIVE)){
        if(txn->GetState()!=TransactionState::COMMITTED&&txn->GetState()!=TransactionState::ABORTED){
          txn->SetState(TransactionState::SHRINKING);
        }
      }
      if(txn->GetIsolationLevel()==IsolationLevel::READ_COMMITTED&&(lock_request->lock_mode_==LockMode::EXCLUSIVE)){
        if(txn->GetState()!=TransactionState::COMMITTED&&txn->GetState()!=TransactionState::ABORTED){
          txn->SetState(TransactionState::SHRINKING);
        }
      }
      if(txn->GetIsolationLevel()==IsolationLevel::READ_UNCOMMITTED&&(lock_request->lock_mode_==LockMode::EXCLUSIVE)){
        if(txn->GetState()!=TransactionState::COMMITTED&&txn->GetState()!=TransactionState::ABORTED){
          txn->SetState(TransactionState::SHRINKING);
        }
      }
...
void LockManager::InsertOrDeleteTableLockSet(Transaction *txn,std::shared_ptr<LockRequest> &lock_request,bool insert){
  switch(lock_request->lock_mode_){
    case LockMode::SHARED:
      if(insert){
...
auto LockManager::GrantLock(std::shared_ptr<LockRequest> &lock_request,std::shared_ptr<LockRequestQueue> &lock_request_queue,bool upgrading)->bool{
  for(auto request :lock_request_queue->request_queue_){ // NOLINT
    if(request->granted_){
      switch(lock_request->lock_mode_){
        case LockMode::SHARED:
          if(request->lock_mode_==LockMode::SHARED_INTENTION_EXCLUSIVE||request->lock_mode_==LockMode::EXCLUSIVE){
            return false;
          }
          break;
        case LockMode::EXCLUSIVE:
        ...
        case LockMode::INTENTION_SHARED:
          ...
          break;
        case LockMode::INTENTION_EXCLUSIVE:
          if(request->lock_mode_==LockMode::SHARED||request->lock_mode_==LockMode::SHARED_INTENTION_EXCLUSIVE||request->lock_mode_==LockMode::EXCLUSIVE){
            return false;
          }
          break;
        case LockMode::SHARED_INTENTION_EXCLUSIVE:
...
auto LockManager::IsCompatible(std::shared_ptr<LockRequest> &waiting_request,std::shared_ptr<LockRequest> &granted_request)->bool{
   switch(waiting_request->lock_mode_){
        case LockMode::SHARED:
          if(granted_request->lock_mode_==LockMode::SHARED_INTENTION_EXCLUSIVE||granted_request->lock_mode_==LockMode::EXCLUSIVE){
            return false;
          }
          break;
        case LockMode::EXCLUSIVE:
        ...
        case LockMode::INTENTION_SHARED:
          ...
          break;
        case LockMode::INTENTION_EXCLUSIVE:
          if(granted_request->lock_mode_==LockMode::SHARED||granted_request->lock_mode_==LockMode::SHARED_INTENTION_EXCLUSIVE||granted_request->lock_mode_==LockMode::EXCLUSIVE){
            return false;
          }
          break;
        case LockMode::SHARED_INTENTION_EXCLUSIVE:
