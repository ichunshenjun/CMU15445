...
void DeleteExecutor::Init() {
  ...
  try {
    auto lock_success = this->exec_ctx_->GetLockManager()->LockTable(
...
auto DeleteExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
  ...
  while (child_executor_->Next(tuple, rid)) {
    auto delete_success = table_info_->table_->MarkDelete(*rid, exec_ctx_->GetTransaction());
    if (delete_success) {
      try {
        ...
        if (!lock_success) {
          throw ExecutionException("Delete Excutor Get Row Lock Failed");
        }
      } catch (ExecutionException e) {
        throw ExecutionException("Delete Excutor Get Table Lock Failed");
      }
    }
